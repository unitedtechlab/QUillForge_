...
const verifyjwt= asyncHandler(async (req,res,next)=>{
    ...
    const token = req.cookies?.accessToken || req.headers.authorization?.split(" ")[1];
      //reading cookies here and also from headers for flexibility and get accesstoken from both places
    if(!token) {
        throw new ApiError(401, "unauthenticated");
    }
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
     //jwt.verify will throw an error if token is invalid or expired, so we can catch that in our global error handler
