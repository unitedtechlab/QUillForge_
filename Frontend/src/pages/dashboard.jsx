...
/* ─────────────────── ANALYTICS PAGE ─────────────────── */
function AnalyticsPage({ userBlogs }) {
  const blogs = userBlogs || [];

  const totalViews = blogs.reduce((s, b) => s + (b.views || 0), 0);
  const totalLikes = blogs.reduce((s, b) => s + (Array.isArray(b.likes) ? b.likes.length : (b.likes || 0)), 0);
  ...
  // Views over the last 6 months, bucketed by blog creation month
  const monthlyData = (() => {
    const months = [];
    const now = new Date();
    for (let i = 5; i >= 0; i--) {
  ...
  const topPosts = [...blogs]
    ...
    .slice(0, 5)
    .map(b => ({
      name: b.title && b.title.length > 22 ? b.title.slice(0, 22) + "…" : (b.title || "Untitled"),
      views: b.views || 0
    }));
  ...
  const PIE_COLORS = ["#8F72FF", "#2ac3de", "#FF728F", "#ff9e64", "#a3e635", "#f472b6"];

  const chartCard = "border-2 border-retro-border bg-retro-surface rounded-2xl p-5 shadow-[6px_6px_0px_0px_#1C1D2E]";
  const labelCls = "text-xs font-pixel uppercase tracking-widest text-retro-accent mb-4";
  ...
  const stats = [
    { label: "Total Views", value: totalViews, color: "text-cyan-400" },
    { label: "Total Likes", value: totalLikes, color: "text-orange-400" },
    { label: "Engagement", value: `${engagementRate}%`, color: "text-retro-accent" },
    { label: "Published", value: `${published}/${blogs.length}`, color: "text-[#a3e635]" }
  ];

  if (blogs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
        <div className="border-2 border-retro-border bg-retro-surface p-8 shadow-[6px_6px_0px_0px_#1C1D2E] rounded-2xl max-w-md">
          <BarChart3 size={28} className="text-retro-accent mx-auto mb-4" />
          <h2 className="text-2xl font-black text-retro-accent mb-2 uppercase tracking-widest font-heading">No data yet</h2>
          <p className="text-retro-text/60 text-sm font-terminal uppercase">Publish some blogs to see your analytics come to life.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl md:text-4xl font-black text-retro-accent uppercase tracking-widest font-heading">Analytics</h1>
        <p className="text-retro-text/40 text-xs font-terminal uppercase tracking-widest mt-1">Your writing performance at a glance</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          <div key={i} className="border-2 border-retro-border bg-retro-surface rounded-2xl p-5 shadow-[4px_4px_0px_0px_#1C1D2E]">
            <p className="text-[11px] font-pixel uppercase tracking-widest text-retro-text/40 mb-2">{s.label}</p>
            <p className={`text-3xl font-black ${s.color} font-heading`}>{s.value}</p>
          </div>
        ))}
      </div>

      <div className={chartCard}>
        <p className={labelCls}>Views Over Time</p>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={monthlyData} margin={{ top: 10, right: 10, left: -15, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="views" stroke="#8F72FF" strokeWidth={2} fill="url(#viewsGrad)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className={chartCard}>
          <p className={labelCls}>Top Posts by Views</p>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={topPosts} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1C1D2E" horizontal={false} />
              <XAxis type="number" stroke="#E2E2F5" tick={{ fontSize: 11, fontFamily: "monospace" }} allowDecimals={false} />
              <YAxis type="category" dataKey="name" stroke="#E2E2F5" tick={{ fontSize: 10, fontFamily: "monospace" }} width={110} />
              <Tooltip contentStyle={tooltipStyle} cursor={{ fill: "#1C1D2E" }} />
              <Bar dataKey="views" fill="#2ac3de" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className={chartCard}>
          <p className={labelCls}>Posts by Category</p>
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie
                data={categoryData}
      ...
      <div className={chartCard}>
        <p className={labelCls}>Draft vs Published</p>
        <div className="flex items-center gap-4">
          <div className="flex-1 h-6 rounded-full overflow-hidden bg-[#13141f] border border-retro-border flex">
            <div
              className="bg-[#a3e635] h-full flex items-center justify-center text-[10px] font-pixel text-black"
              style={{ width: `${blogs.length ? (published / blogs.length) * 100 : 0}%` }}
            >
              {published > 0 && `${published}`}
            </div>
            <div
              className="bg-retro-accent h-full flex items-center justify-center text-[10px] font-pixel text-black"
              style={{ width: `${blogs.length ? (drafts / blogs.length) * 100 : 0}%` }}
            >
              {drafts > 0 && `${drafts}`}
            </div>
          </div>
          <div className="flex gap-4 text-[11px] font-terminal uppercase">
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-[#a3e635]" /> Published</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-retro-accent" /> Drafts</span>
          </div>
        </div>
      </div>
    </div>
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-retro-bg text-retro-text relative overflow-x-hidden">
      ...
      {/* Main content */}
      <main
        className="relative z-10 min-h-screen pt-28 pb-8 pr-4 transition-all duration-300"
      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          {/* Analytics Page */}
          {active === "analytics" && (
            <AnalyticsPage userBlogs={userBlogs} />
          )}

          {/* Placeholder/Alternative Pages */}
          {active !== "dashboard" && active !== "create" && active !== "ai-assistant" && active !== "blogs" && active !== "read" && active !== "analytics" && (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
              <div className="border-2 border-retro-border bg-retro-surface p-10 shadow-[6px_6px_0px_0px_#1C1D2E] rounded-2xl text-center max-w-md mx-auto">
                ...
                <div className="inline-block mb-3 px-3 py-1 border border-retro-accent/40 bg-retro-accent/10 rounded-full">
                  <span className="text-[10px] font-pixel uppercase tracking-widest text-retro-accent">Coming Soon</span>
                </div>
                <h2 className="text-3xl font-black text-retro-accent mb-3 uppercase tracking-widest font-heading">
                  {active.toUpperCase()}
                </h2>
                <p className="text-retro-text/60 text-sm font-terminal uppercase leading-relaxed">
                  Follow writers, comment on posts, and build your reading circle. We're forging it now.
                </p>
              </div>
            </div>
          )}
