...
function DashboardPage({ setPage }) {
  ...
  return (
    <motion.div key="dashboard" {...pageVariants} variants={undefined} initial="initial" animate="animate" exit="exit" className="space-y-6">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{delay:0.25}}>
          <GlassCard>
            ...
            <div className="p-4 space-y-1.5">
              {BLOGS.map((b,i)=>(
                <motion.div key={b.id} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}} transition={{delay:0.3+i*0.06}}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/20 text-[10px] mt-0.5" style={{fontFamily:T.mono}}>{b.category} · {b.created}</p>
                  </div>
...
function CreateBlogPage() {
  ...
  const Field = ({ label, children }) => (
    <div className="space-y-2">
      <label className="block text-[10px] font-semibold text-white/30 tracking-widest uppercase" style={{fontFamily:T.ox}}>{label}</label>
      {children}
    </div>
  );
  ...
  return (
    <motion.div key="create" initial="initial" animate="animate" exit="exit" variants={pageVariants} className="space-y-5">
      {/* Header */}
      <motion.div variants={fadeUp} initial="hidden" animate="show">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {saved && (
            <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0}}
              className="flex items-center gap-2 text-emerald-400 text-xs border border-emerald-400/25 bg-emerald-400/10 px-3 py-2 rounded-xl" style={{fontFamily:T.mono}}>
      ...
      <div className="grid xl:grid-cols-[1fr_280px] gap-5">
        {/* Main editor */}
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{delay:0.08}} className="space-y-4">
          <GlassCard className="p-6 space-y-5" hover={false}>
            {/* Blog Title */}
            <Field label="Blog Title">
              <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Write a compelling title…"
                className={`${inputCls} text-lg font-bold`} style={{fontFamily:T.ox}}/>
            </Field>
        ...
        {/* Sidebar */}
        <motion.div variants={fadeLeft} initial="hidden" animate="show" transition={{delay:0.14}} className="space-y-4">
...
function ManageBlogsPage() {
  ...
  const filtered = blogs
    ...
    .filter(b => b.title.toLowerCase().includes(search.toLowerCase()) || b.category.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <motion.div key="manage" initial="initial" animate="animate" exit="exit" variants={pageVariants} className="space-y-5">
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-white/25 text-xs mt-1" style={{fontFamily:T.mono}}>{blogs.length} total · {blogs.filter(b=>b.status==="published").length} published · {blogs.filter(b=>b.status==="draft").length} drafts</p>
          </div>
                  ...
                  <div className="hidden lg:grid grid-cols-[24px_1fr_100px_80px_100px_100px_100px] gap-3 px-4 py-4 items-center">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-white/20 text-[10px]" style={{fontFamily:T.mono}}>{b.category}</p>
                      </div>
                    </div>
                    ...
                    <span className="text-white/50 text-xs flex items-center gap-1" style={{fontFamily:T.mono}}>
                      <Eye size={9} className="text-violet-400"/>{b.status==="published"?(b.views>=1000?`${(b.views/1000).toFixed(1)}K`:b.views):"—"}
                    </span>

                    <span className="text-white/30 text-[10px]" style={{fontFamily:T.mono}}>{b.created}</span>
                    <span className="text-white/30 text-[10px]" style={{fontFamily:T.mono}}>{b.updated}</span>
                    ...
                    <div className="relative">
                      <div className="flex items-center gap-1">
                        {[
                          ...
                          { icon:<Trash2 size={11}/>,       title:"Delete",  cls:"hover:text-red-400 hover:bg-red-400/10 hover:border-red-400/20", action:()=>deleteB(b.id) },
                        ].map((a,j)=>(
                  ...
                  <div className="lg:hidden p-4 flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge status={b.status}/>
                        <span className="text-white/30 text-[10px]" style={{fontFamily:T.mono}}>{b.category}</span>
                        {b.status==="published" && <span className="text-violet-400 text-[10px] flex items-center gap-1" style={{fontFamily:T.mono}}><Eye size={9}/>{(b.views/1000).toFixed(1)}K</span>}
                      </div>
                      <div className="flex items-center gap-2">
                        {[{i:<Edit3 size={11}/>,c:"text-cyan-400"},{i:<ExternalLink size={11}/>,c:"text-violet-400"},{i:<Trash2 size={11}/>,c:"text-red-400",fn:()=>deleteB(b.id)}].map((a,j)=>(
...
function PlaceholderPage({ title, icon, desc }) {
  return (
    <motion.div key={title} initial="initial" animate="animate" exit="exit" variants={pageVariants}
...
useEffect(() => {
  ...
  const renderPage = () => {
    switch(page) {
      case "dashboard": return <DashboardPage   key="dashboard" setPage={setPage}/>;
      case "create":    return <CreateBlogPage  key="create"/>;
      case "manage":    return <ManageBlogsPage key="manage"/>;
      case "analytics": return <PlaceholderPage key="analytics" title="Analytics"  icon={<BarChart3 size={24}/>}  desc="Detailed performance charts coming soon."/>;
      case "settings":  return <PlaceholderPage key="settings"  title="Settings"   icon={<Settings size={24}/>}   desc="Profile and account settings coming soon."/>;
